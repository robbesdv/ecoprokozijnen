...
export default function BeheerPage() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ background: 'white', borderBottom: '1px solid var(--border)', padding: '20px 24px 16px', flexShrink: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 14, marginBottom: 20 }}>
          {[
            ...
            { label: 'Gerealiseerde omzet', value: formatEuro(stats.gerealiseerd), icon: '✓', sub: `${stats.realisatiePct}% van totaal`, accent: 'var(--success)', isText: true, progress: stats.realisatiePct, onClick: () => setShowOmzetModal(true), actionLabel: 'Bekijk details →' },
          ].map(s => (
            ...
            >
              ...
              {s.onClick && <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: s.accent, marginTop: 5, opacity: 0.8 }}>{s.actionLabel || 'Filteren →'}</div>}
            </div>
          ))}
        </div>
      ...
      {showOmzetModal && (
        <OmzetBreakdownModal orders={orders} onClose={() => setShowOmzetModal(false)} />
      )}
...
function OmzetBreakdownModal({ orders, onClose }) {
  const rows = orders
    .map(o => ({ ...o, realized: getRealizedRevenue(o) }))
    .filter(o => o.realized > 0)
    .sort((a, b) => b.realized - a.realized)

  const byVerkoper = rows.reduce((acc, o) => {
    const key = o.sales_owner || '__geen__'
    if (!acc[key]) acc[key] = { name: VERKOPER_NAMEN[key] || (key === '__geen__' ? 'Onbekend / EcoPro' : key), orders: [], total: 0 }
    acc[key].orders.push(o)
    acc[key].total += o.realized
    return acc
  }, {})

  const totalRealized = rows.reduce((s, o) => s + o.realized, 0)

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: 'white', borderRadius: 18, width: '100%', maxWidth: 680, maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg, #152318, #1e3a28)', color: 'white', padding: '22px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 18 }}>Gerealiseerde omzet</div>
            <div style={{ fontSize: 13, opacity: 0.65, marginTop: 3 }}>Ontvangen betalingen per order en verkoper</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, opacity: 0.6 }}>Totaal</div>
              <div style={{ fontWeight: 800, fontSize: 20 }}>{formatEuro(totalRealized)}</div>
            </div>
            <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          </div>
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 26px' }}>
          {rows.length === 0 && (
            <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '32px 0' }}>Nog geen gerealiseerde omzet.</p>
          )}
          {Object.entries(byVerkoper).map(([key, group]) => (
            <div key={key} style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--success)' }}>{formatEuro(group.total)}</div>
              </div>
              <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                {group.orders.map((o, i) => {
                  const p = getPhase(o.phase)
                  return (
                    <div key={o.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 12, padding: '10px 14px', borderBottom: i < group.orders.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'center', fontSize: 13 }}>
                      <div>
                        <div style={{ fontWeight: 600 }}>{o.customer_name}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>{o.customer_address || '—'}</div>
                      </div>
                      <span className={`badge ${p.badgeClass}`}>{p.adminLabel}</span>
                      <div style={{ textAlign: 'right', minWidth: 90 }}>
                        <div style={{ fontWeight: 700, color: 'var(--success)', fontSize: 14 }}>{formatEuro(o.realized)}</div>
                        {o.realized < o.total_amount && (
                          <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>van {formatEuro(o.total_amount)}</div>
                        )}
                      </div>
                    </div>
                  )
        ...
        <div style={{ padding: '14px 26px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, background: '#FAFAFA' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)' }}>{rows.length} orders met gerealiseerde omzet</span>
