...
const NAV = [
  { section: 'Overzicht', items: [
    ...
    { href: '/beheer?view=klanten', label: 'Klanten',   icon: '👥', exact: true, view: 'klanten' },
    { href: '/beheer/leads',        label: 'Leads',     icon: '☎', exact: true },
    { href: '/beheer/agenda',       label: 'Agenda',    icon: '📅', exact: true },
  ]},
  { section: 'Werkplekken', items: [
    { href: '/beheer/verkoop',      label: 'Verkoop',      icon: '💼' },
    { href: '/beheer/dakkapellab',  label: 'DakkapelLAB',  icon: '🏠' },
...
export default function BeheerNav({ topSlot }) {
  ...
  function isActive(item) {
    if (item.view) return path === '/beheer' && view === item.view
    const cleanHref = item.href.split('?')[0]
    if (cleanHref === '/beheer') return path === '/beheer' && !view
    return item.exact ? path === cleanHref : path.startsWith(cleanHref)
  }

  return (
    <div style={{ width: 220, background: '#152318', color: 'white', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100dvh', borderRight: '1px solid rgba(0,0,0,0.2)' }}>
