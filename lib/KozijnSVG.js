  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (pType === 'deur') {
        ...
        pushDoorLeafPanels(sx, sy, sw, sh, doorPanelsFor(el, row), mullW)
        ...
        pushFlagHinges(hinge === 'left' ? 'left' : 'right', hinge === 'left' ? cxPx : cxPx + cwPx, cyPos, rh, mullW)
      }
      if (pType === 'deur2') {
        ...
        parts.push(<rect key={k++} x={centerX - handleW - Math.max(2, mullW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        parts.push(<rect key={k++} x={centerX + Math.max(2, mullW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        // Scharnieren op grens kozijn/deurprofiel, beide kanten
        pushFlagHinges('left', cxPx, cyPos, rh, mullW)
        pushFlagHinges('right', cxPx + cwPx, cyPos, rh, mullW)
      }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (pType === 'deur2') {
        ...
        const handleY = sy + sh * 0.5 - handleH / 2
        parts.push(`<rect x="${centerX - handleW - handleGap}" y="${handleY}" width="${handleW}" height="${handleH}" fill="#334155" rx="1"/>`)
        parts.push(`<rect x="${centerX + handleGap}" y="${handleY}" width="${handleW}" height="${handleH}" fill="#334155" rx="1"/>`)
        pushFlagHinges('left', cxPx, cyPos, rh, mullW)
        pushFlagHinges('right', cxPx + cwPx, cyPos, rh, mullW)
      }
      if (['kiep', 'draaikiep'].includes(pType)) {
