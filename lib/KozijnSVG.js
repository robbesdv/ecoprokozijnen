...
const RAL_NAMES = {
  ...
  RAL5011: 'Staalblauw',
  RAL3009: 'Oxidrood',
  DB703:       'DB Zilvergrijs',
  GoudenEik:   'Gouden Eik',
  IersEik:     'Iers Eik',
  DonkerEik:   'Donker Eik (Chêne Foncé)',
  Palissander: 'Palissander',
  Mahonie:     'Mahonie',
}
...
function normalizeEl(el) {
  ...
  const normalized = el.widthMM !== undefined ? { ...el } : {
    ...
    colorSash: el.finish?.colorSash || el.colorSash,
...
function defaultDoorPanel(fill = 'panel') {
  return { heightPct: 100, fill, panelPack: 'HR++', glassPack: 'HR++', glassFinish: 'clear' }
}
...
export function KozijnSVG({ element, width = 420, height = 280, showDims = true }) {
  ...
  const sashColorHex = RAL[sashCode] || colorHex
  const isLightColor = ['#f6f6f3','#f0ead6','#ebe2c8','#e8dcc9','#d3d3cb'].includes(colorHex)
  const borderColor = shadeHex(colorHex, isLightColor ? -60 : -25)
    ...
    panels.forEach((panel, i) => {
      ...
      if (panel.fill === 'glass') {
        ...
        if (isMatteGlassFinish(panel.glassFinish)) {
          for (let ly = cy + 3; ly < cy + ph - 2; ly += 6) {
            parts.push(<line key={k++} x1={innerX + 3} y1={ly} x2={innerX + innerW - 3} y2={ly} stroke="rgba(255,255,255,.65)" strokeWidth="0.45" />)
          }
        }
      } else {
        const pFill = shadeHex(colorHex, 14), pStroke = shadeHex(colorHex, -22)
        parts.push(<rect key={k++} x={innerX} y={cy} width={innerW} height={ph} fill={pFill} stroke={pStroke} strokeWidth="0.8" rx="1" />)
        parts.push(<rect key={k++} x={innerX + 4} y={cy + 4} width={Math.max(1, innerW - 8)} height={Math.max(1, ph - 8)} fill="none" stroke={pStroke} strokeWidth="0.6" rx="1" />)
      }
      cy += ph + gap
    })
  }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (isDoorPane) {
        // Door: sash color covers full pane area
        parts.push(<rect key={k++} x={cxPx + 2} y={cyPos + 2} width={cwPx - 4} height={rh - 4} fill={sashColorHex} stroke={shadeHex(sashColorHex, -30)} strokeWidth="0.8" rx="1" />)
      } else if (openable) {
        // Openable kozijn pane: profile background at outer edge, glass inside at inset
        parts.push(<rect key={k++} x={cxPx + 2} y={cyPos + 2} width={cwPx - 4} height={rh - 4} fill={shadeHex(sashColorHex, -8)} stroke={shadeHex(sashColorHex, -30)} strokeWidth="0.8" rx="1" />)
        parts.push(<rect key={k++} x={sx} y={sy} width={sw} height={sh} fill="#dbeafe" strokeWidth="0" />)
        if (row.fill !== 'panel' && isMatteGlassFinish(row.glassFinish)) {
          for (let ly = sy + 3; ly < sy + sh - 2; ly += 6) {
            parts.push(<line key={k++} x1={sx + 3} y1={ly} x2={sx + sw - 3} y2={ly} stroke="rgba(255,255,255,.65)" strokeWidth="0.45" />)
          }
        }
      }
      if (openable) {
        const sashStroke = shadeHex(sashColorHex, -40)
        parts.push(<rect key={k++} x={sx} y={sy} width={sw} height={sh} fill="none" stroke={sashStroke} strokeWidth="0.9" />)
      }
      ...
      if (pType === 'schuif') {
        ...
        const dir = row.slideDirection || slideDirectionFor(cols, ci)
        parts.push(<line key={k++} x1={ax1} y1={ay} x2={ax2} y2={ay} stroke="#334155" strokeWidth="1.2" />)
        parts.push(<path key={k++} d={dir === 'left' ? `M ${ax1 + 7} ${ay - 4} L ${ax1} ${ay} L ${ax1 + 7} ${ay + 4}` : `M ${ax2 - 7} ${ay - 4} L ${ax2} ${ay} L ${ax2 - 7} ${ay + 4}`} fill="none" stroke="#334155" strokeWidth="1" strokeLinejoin="round" />)
      }
      if (pType === 'vast' && row.fill !== 'panel') {
        ...
        if (isMatteGlassFinish(row.glassFinish)) {
          for (let ly = cyPos + 4; ly < cyPos + rh - 2; ly += 6) {
            parts.push(<line key={k++} x1={cxPx + 6} y1={ly} x2={cxPx + cwPx - 6} y2={ly} stroke="rgba(255,255,255,.65)" strokeWidth="0.45" />)
          }
        }
        parts.push(
...
export function kozijnSVGString(element, width = 420, height = 280) {
  ...
  function pushDoorLeafPanels(lx, ly, lw, lh, panels, profileW) {
    ...
    panels.forEach((panel, i) => {
      ...
      if (panel.fill === 'glass') {
        ...
        if (isMatteGlassFinish(panel.glassFinish)) {
          for (let ly = cy + 3; ly < cy + ph - 2; ly += 6) {
            parts.push(`<line x1="${innerX + 3}" y1="${ly}" x2="${innerX + innerW - 3}" y2="${ly}" stroke="rgba(255,255,255,.65)" stroke-width="0.45"/>`)
          }
        }
      } else {
        const pFill = shadeHex(colorHex, 14), pStroke = shadeHex(colorHex, -22)
        parts.push(`<rect x="${innerX}" y="${cy}" width="${innerW}" height="${ph}" fill="${pFill}" stroke="${pStroke}" stroke-width="0.8" rx="1"/>`)
        parts.push(`<rect x="${innerX + 4}" y="${cy + 4}" width="${Math.max(1, innerW - 8)}" height="${Math.max(1, ph - 8)}" fill="none" stroke="${pStroke}" stroke-width="0.6" rx="1"/>`)
      }
      cy += ph + gap
    })
  }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      const openableStr = ['draai', 'draaikiep', 'deur', 'deur2', 'kiep', 'schuif'].includes(pType)
      if (isDoorPane) {
        parts.push(`<rect x="${cxPx + 2}" y="${cyPos + 2}" width="${cwPx - 4}" height="${rh - 4}" fill="${sashColorHex}" stroke="${shadeHex(sashColorHex, -30)}" stroke-width="0.8" rx="1"/>`)
      } else if (openableStr) {
        parts.push(`<rect x="${cxPx + 2}" y="${cyPos + 2}" width="${cwPx - 4}" height="${rh - 4}" fill="${shadeHex(sashColorHex, -8)}" stroke="${shadeHex(sashColorHex, -30)}" stroke-width="0.8" rx="1"/>`)
        parts.push(`<rect x="${sx}" y="${sy}" width="${sw}" height="${sh}" fill="#dbeafe" stroke="none"/>`)
        if (row.fill !== 'panel' && isMatteGlassFinish(row.glassFinish)) {
          for (let ly = sy + 3; ly < sy + sh - 2; ly += 6) {
            parts.push(`<line x1="${sx + 3}" y1="${ly}" x2="${sx + sw - 3}" y2="${ly}" stroke="rgba(255,255,255,.65)" stroke-width="0.45"/>`)
          }
        }
      }
      if (openableStr) {
        parts.push(`<rect x="${sx}" y="${sy}" width="${sw}" height="${sh}" fill="none" stroke="${shadeHex(sashColorHex, -40)}" stroke-width="0.9"/>`)
      }
      if (pType === 'draai' || pType === 'draaikiep') {
        const hx = hinge === 'left' ? sx : sx + sw
        const ox = hinge === 'left' ? sx + sw : sx
      ...
      if (pType === 'schuif') {
        ...
        parts.push(`<path d="${dir === 'left' ? `M ${ax1 + 7} ${ay - 4} L ${ax1} ${ay} L ${ax1 + 7} ${ay + 4}` : `M ${ax2 - 7} ${ay - 4} L ${ax2} ${ay} L ${ax2 - 7} ${ay + 4}`}" fill="none" stroke="#334155" stroke-width="1" stroke-linejoin="round"/>`)
      }
      if (pType === 'vast' && row.fill !== 'panel') {
        const cx2 = cxPx + cwPx / 2, cy2 = cyPos + rh / 2
        const sz = Math.min(cwPx, rh) * 0.06
        if (isMatteGlassFinish(row.glassFinish)) {
          for (let ly = cyPos + 4; ly < cyPos + rh - 2; ly += 6) {
            parts.push(`<line x1="${cxPx + 6}" y1="${ly}" x2="${cxPx + cwPx - 6}" y2="${ly}" stroke="rgba(255,255,255,.65)" stroke-width="0.45"/>`)
          }
        }
