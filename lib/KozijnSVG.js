...
function computeGeom(el, vw, vh) {
  ...
  const prof = el.profile || { frameMM: 70, mullionMM: 60, transomMM: 60 }
...
export function KozijnSVG({ element, width = 420, height = 280, showDims = true }) {
  ...
  const { fx, fy, fw, fh, ix, iy, iw, ih, mullW, transW, cols, colWidths, colXs } = computeGeom(el, width, height)
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (['deur', 'deur2'].includes(pType)) {
        // Dorpel onderaan
        const dorpelH = Math.max(8, mullW * 1.5)
        parts.push(<rect key={k++} x={cxPx} y={cyPos + rh - dorpelH} width={cwPx} height={dorpelH} fill={colorHex} stroke={borderColor} strokeWidth="0.7" />)
        // Vlagscharnieren
        const scharH = Math.max(12, rh * 0.09)
        const scharW = Math.max(4, mullW * 0.85)
        const hingePositions = [cyPos + rh * 0.22, cyPos + rh * 0.72]
        if (pType === 'deur') {
          const scharX = hinge === 'left' ? cxPx : cxPx + cwPx - scharW
          hingePositions.forEach(hy => {
            parts.push(<rect key={k++} x={scharX} y={hy - scharH / 2} width={scharW} height={scharH} fill="#607d8b" rx="1" />)
          })
        }
        if (pType === 'deur2') {
          hingePositions.forEach(hy => {
            parts.push(<rect key={k++} x={cxPx} y={hy - scharH / 2} width={scharW} height={scharH} fill="#607d8b" rx="1" />)
            parts.push(<rect key={k++} x={cxPx + cwPx - scharW} y={hy - scharH / 2} width={scharW} height={scharH} fill="#607d8b" rx="1" />)
          })
        }
      }
...
export function kozijnSVGString(element, width = 420, height = 280) {
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (['deur', 'deur2'].includes(pType)) {
        const dorpelH = Math.max(8, mullW * 1.5)
        parts.push(`<rect x="${cxPx}" y="${cyPos + rh - dorpelH}" width="${cwPx}" height="${dorpelH}" fill="${colorHex}" stroke="${borderColor}" stroke-width="0.7"/>`)
        const scharH = Math.max(12, rh * 0.09)
        const scharW = Math.max(4, mullW * 0.85)
        const hingePositions = [cyPos + rh * 0.22, cyPos + rh * 0.72]
        if (pType === 'deur') {
          const scharX = hinge === 'left' ? cxPx : cxPx + cwPx - scharW
          hingePositions.forEach(hy => {
            parts.push(`<rect x="${scharX}" y="${hy - scharH / 2}" width="${scharW}" height="${scharH}" fill="#607d8b" rx="1"/>`)
          })
        }
        if (pType === 'deur2') {
          hingePositions.forEach(hy => {
            parts.push(`<rect x="${cxPx}" y="${hy - scharH / 2}" width="${scharW}" height="${scharH}" fill="#607d8b" rx="1"/>`)
            parts.push(`<rect x="${cxPx + cwPx - scharW}" y="${hy - scharH / 2}" width="${scharW}" height="${scharH}" fill="#607d8b" rx="1"/>`)
          })
        }
      }
      if (['kiep', 'draaikiep'].includes(pType)) {
