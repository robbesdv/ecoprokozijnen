  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (pType === 'deur') {
        ...
        pushFlagHinges(hinge === 'left' ? 'left' : 'right', hinge === 'left' ? cxPx : cxPx + cwPx, cyPos, rh, doorPaneProfileW)
      }
      if (pType === 'deur2') {
        ...
        const handleW = Math.max(2.5, doorPaneProfileW * 0.18)
        const handleY = sy + sh * 0.5 - handleH / 2
        parts.push(<rect key={k++} x={centerX - handleW - Math.max(2, doorPaneProfileW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        parts.push(<rect key={k++} x={centerX + Math.max(2, doorPaneProfileW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        // Scharnieren op grens kozijn/deurprofiel, beide kanten
        pushFlagHinges('left', cxPx, cyPos, rh, doorPaneProfileW)
        pushFlagHinges('right', cxPx + cwPx, cyPos, rh, doorPaneProfileW)
      }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (pType === 'deur2') {
        ...
        const handleH = Math.max(10, sh * 0.045)
        const handleW = Math.max(2.5, doorPaneProfileW * 0.18)
        const handleGap = Math.max(2, doorPaneProfileW * 0.15)
        const handleY = sy + sh * 0.5 - handleH / 2
