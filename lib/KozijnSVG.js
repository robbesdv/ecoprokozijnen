...
function normalizeEl(el) {
  if (!el) return el
  if (el.widthMM !== undefined) return el
  return {
    ...el,
...
export function KozijnSVG({ element, width = 420, height = 280, showDims = true }) {
  ...
  function pushDoorLeafPanels(lx, ly, lw, lh, panels, profileW) {
    ...
    const gap = panels.length > 1 ? Math.max(2, Math.min(9, profileW * 0.24)) : 0
    ...
    panels.forEach((panel, i) => {
      ...
      if (i < panels.length - 1) {
        parts.push(<rect key={k++} x={lx + rail * 0.45} y={cy + ph} width={lw - rail * 0.9} height={gap} fill="#334155" rx="1" />)
      }
      cy += ph + gap
    })
  }

  function pushFlagHinges(side, sashEdgeX, y, h, profileW) {
    const halfW = Math.max(3, Math.min(9, profileW * 0.22))
    const hingeH = Math.max(9, Math.min(24, h * 0.075))
    ;[0.18, 0.5, 0.82].forEach(pos => {
      const hy = y + h * pos - hingeH / 2
      if (side === 'left') {
        parts.push(<rect key={k++} x={sashEdgeX - halfW - 1} y={hy} width={halfW} height={hingeH} fill="#475569" rx="1" />)
        parts.push(<rect key={k++} x={sashEdgeX} y={hy} width={halfW} height={hingeH} fill="#334155" rx="1" />)
      } else {
        parts.push(<rect key={k++} x={sashEdgeX - halfW} y={hy} width={halfW} height={hingeH} fill="#334155" rx="1" />)
        parts.push(<rect key={k++} x={sashEdgeX + 1} y={hy} width={halfW} height={hingeH} fill="#475569" rx="1" />)
      }
    })
  }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      const inset  = Math.max(3, Math.min(isDoorPane ? 20 : 10, Math.min(cwPx, rh) * (isDoorPane ? 0.14 : 0.09)))
      ...
      if (['draai', 'draaikiep', 'deur'].includes(pType)) {
        ...
        if (pType === 'deur') pushDoorLeafPanels(sx, sy, sw, sh, doorPanelsFor(el, row), mullW)
        parts.push(<path key={k++} d={`M ${hx} ${sy} L ${ox} ${sy + sh / 2} L ${hx} ${sy + sh}`} fill="none" stroke="#334155" strokeWidth="0.9" strokeLinejoin="round" />)
      }
      if (pType === 'deur2') {
        const mxD = sx + sw / 2
        parts.push(<line key={k++} x1={mxD} y1={sy} x2={mxD} y2={sy + sh} stroke="#334155" strokeWidth="0.8" />)
        const gap = Math.max(2, inset / 2)
        const lw = sw / 2 - gap / 2
        const rox = sx + sw / 2 + gap / 2
        const rw = sw - lw - gap
        const panels = doorPanelsFor(el, row)
        pushDoorLeafPanels(sx, sy, lw, sh, panels, mullW)
        pushDoorLeafPanels(rox, sy, rw, sh, panels, mullW)
        parts.push(<path key={k++} d={`M ${sx} ${sy} L ${mxD} ${sy + sh / 2} L ${sx} ${sy + sh}`} fill="none" stroke="#334155" strokeWidth="0.9" strokeLinejoin="round" />)
        parts.push(<path key={k++} d={`M ${sx + sw} ${sy} L ${mxD} ${sy + sh / 2} L ${sx + sw} ${sy + sh}`} fill="none" stroke="#334155" strokeWidth="0.9" strokeLinejoin="round" />)
        const handleH = Math.max(10, sh * 0.045)
        const handleW = Math.max(2.5, mullW * 0.18)
        const handleY = sy + sh * 0.5 - handleH / 2
        parts.push(<rect key={k++} x={mxD - handleW - Math.max(2, mullW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        parts.push(<rect key={k++} x={mxD + Math.max(2, mullW * 0.15)} y={handleY} width={handleW} height={handleH} fill="#334155" rx="1" />)
        pushFlagHinges('left', sx, sy, sh, mullW)
        pushFlagHinges('right', sx + sw, sy, sh, mullW)
      }
      if (pType === 'deur') {
        pushFlagHinges(hinge === 'left' ? 'left' : 'right', hinge === 'left' ? sx : sx + sw, sy, sh, mullW)
      }
...
export function kozijnSVGString(element, width = 420, height = 280) {
  ...
  function pushDoorLeafPanels(lx, ly, lw, lh, panels, profileW) {
    ...
    panels.forEach((panel, i) => {
      ...
      if (i < panels.length - 1) {
        parts.push(`<rect x="${lx + rail * 0.45}" y="${cy + ph}" width="${lw - rail * 0.9}" height="${gap}" fill="#334155" rx="1"/>`)
      }
      cy += ph + gap
    })
  }
  ...
  function pushFlagHinges(side, sashEdgeX, y, h, profileW) {
    ...
    ;[0.18, 0.5, 0.82].forEach(pos => {
      const hy = y + h * pos - hingeH / 2
      if (side === 'left') {
        parts.push(`<rect x="${sashEdgeX - halfW - 1}" y="${hy}" width="${halfW}" height="${hingeH}" fill="#475569" rx="1"/>`)
        parts.push(`<rect x="${sashEdgeX}" y="${hy}" width="${halfW}" height="${hingeH}" fill="#334155" rx="1"/>`)
      } else {
        parts.push(`<rect x="${sashEdgeX - halfW}" y="${hy}" width="${halfW}" height="${hingeH}" fill="#334155" rx="1"/>`)
        parts.push(`<rect x="${sashEdgeX + 1}" y="${hy}" width="${halfW}" height="${hingeH}" fill="#475569" rx="1"/>`)
      }
    })
  }
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (pType === 'deur2') {
        const mxD = sx + sw / 2
        parts.push(`<line x1="${mxD}" y1="${sy}" x2="${mxD}" y2="${sy + sh}" stroke="#334155" stroke-width="0.8"/>`)
        const gap = Math.max(2, inset / 2)
        const lw = sw / 2 - gap / 2
        const rox = sx + sw / 2 + gap / 2
        const rw = sw - lw - gap
        ...
        pushDoorLeafPanels(sx, sy, lw, sh, panels, mullW)
        pushDoorLeafPanels(rox, sy, rw, sh, panels, mullW)
        parts.push(`<path d="M ${sx} ${sy} L ${mxD} ${sy + sh / 2} L ${sx} ${sy + sh}" fill="none" stroke="#334155" stroke-width="0.9" stroke-linejoin="round"/>`)
        parts.push(`<path d="M ${sx + sw} ${sy} L ${mxD} ${sy + sh / 2} L ${sx + sw} ${sy + sh}" fill="none" stroke="#334155" stroke-width="0.9" stroke-linejoin="round"/>`)
        const handleH = Math.max(10, sh * 0.045)
        const handleW = Math.max(2.5, mullW * 0.18)
        const handleGap = Math.max(2, mullW * 0.15)
        const handleY = sy + sh * 0.5 - handleH / 2
        parts.push(`<rect x="${mxD - handleW - handleGap}" y="${handleY}" width="${handleW}" height="${handleH}" fill="#334155" rx="1"/>`)
        parts.push(`<rect x="${mxD + handleGap}" y="${handleY}" width="${handleW}" height="${handleH}" fill="#334155" rx="1"/>`)
        pushFlagHinges('left', sx, sy, sh, mullW)
        pushFlagHinges('right', sx + sw, sy, sh, mullW)
      }
      if (pType === 'deur') {
