...
export function KozijnSVG({ element, width = 420, height = 280, showDims = true }) {
  ...
  function pushDoorHandle(edgeX, centerY, side, profileW) {
    const plateW = Math.max(2, Math.min(4, profileW * 0.1))
    const plateH = Math.max(9, Math.min(18, profileW * 0.38))
    const leverW = Math.max(7, Math.min(16, profileW * 0.42))
    const leverH = Math.max(1.8, Math.min(3.5, profileW * 0.09))
