...
export function KozijnSVG({ element, width = 420, height = 280, showDims = true }) {
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (isDoorPane) {
        parts.push(<rect key={k++} x={cxPx + 2} y={cyPos + 2} width={cwPx - 4} height={rh - 4} fill={colorHex} stroke={shadeHex(colorHex, -30)} strokeWidth="0.8" rx="1" />)
      }
      if (openable) {
        const sashFill = isDoorPane ? shadeHex(colorHex, -8) : 'none'
        const sashStroke = isDoorPane ? shadeHex(colorHex, -32) : '#334155'
        parts.push(<rect key={k++} x={sx} y={sy} width={sw} height={sh} fill={sashFill} stroke={sashStroke} strokeWidth="0.9" />)
      }
...
export function kozijnSVGString(element, width = 420, height = 280) {
  ...
  cols.forEach((col, ci) => {
    ...
    rows.forEach((row, ri) => {
      ...
      if (isDoorPane) {
        parts.push(`<rect x="${cxPx + 2}" y="${cyPos + 2}" width="${cwPx - 4}" height="${rh - 4}" fill="${colorHex}" stroke="${shadeHex(colorHex, -30)}" stroke-width="0.8" rx="1"/>`)
      }
      if (['draai', 'draaikiep', 'deur', 'deur2', 'kiep', 'schuif'].includes(pType)) {
        const sashFill = isDoorPane ? shadeHex(colorHex, -8) : 'none'
        const sashStroke = isDoorPane ? shadeHex(colorHex, -32) : '#334155'
        parts.push(`<rect x="${sx}" y="${sy}" width="${sw}" height="${sh}" fill="${sashFill}" stroke="${sashStroke}" stroke-width="0.9"/>`)
      }
      if (pType === 'draai' || pType === 'draaikiep') {
        const hx = hinge === 'left' ? sx : sx + sw
        const ox = hinge === 'left' ? sx + sw : sx
